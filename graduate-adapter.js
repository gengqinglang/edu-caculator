...
class GraduateAdapter {
  ...
  getCostData(stage, grade, level, city = '', country = '') {
    try {
      ...
      if (!parsedLevel) {
        console.warn(`GraduateAdapter: 无效的教育水平: ${level}, 国家: ${country}`);
        return null;
      }
  ...
  parseEducationLevel(level, country = '') {
    ...
    if (level === 'overseas') {
      ...
      if (country && overseasCountries.includes(country)) {
        ...
        console.warn(`GraduateAdapter: 海外研究生缺少有效国家信息: ${country}`);
        return null;
      }
    }
  ...
  calculateTotalCost(costData, years) {
    try {
      ...
      console.error('GraduateAdapter.calculateTotalCost error:', error);
      return this.getEmptyCalculationResult(years);
    }
  }
